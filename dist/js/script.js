// Polyfill for Chrome, used in Shadowbox
...
(function($){
	$.confirm = function(params){
		...
		$.each(params.buttons, function (name,obj) {
			...
			buttonHTML += '<a href="#" class="button '+obj['class']+' tw-font">'+name+'<span></span></a>';
		...
		var markup = [
			...
			'<div id="confirmBox">',
			'<header class="tw-font header"><h1>',params.title,'</h1></header>',
			'<p>',params.message,'</p>',
...
var StarRating = function(id) {
	...
	*/
	...
	// Init Splitter
	$('#screen-display').split({orientation:'vertical', limit:125, position:'30%'});
	...
	med.displayRecipe = function(recipe) {
		...
		if (recipe.picture != "" && recipe.picture) {
			...
			recipeDiv.removeClass('no-image');
		} else {
			recipeImageURL = '';
			recipeImage.addClass('hidden-h');
			recipeDiv.addClass('no-image');
		}
